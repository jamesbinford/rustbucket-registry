...
class APIKeyAuthenticationTest(TestCase):
    """Tests for API key authentication in endpoints."""

    def setUp(self):
        self.client = Client()
        self.rustbucket = create_test_rustbucket()
        self.api_key = create_test_api_key(self.rustbucket)

    def test_submit_logs_with_valid_api_key_header(self):
        """Test submitting logs with valid API key in header."""
        response = self.client.post(
            reverse('submit_logs'),
            data=json.dumps({
                'logs': [
                    {'type': 'Info', 'message': 'Test log'}
                ]
            }),
    ...
    def test_submit_logs_with_valid_api_key_body(self):
        """Test submitting logs with valid API key in body."""
        response = self.client.post(
            reverse('submit_logs'),
            data=json.dumps({
    ...
    def test_submit_logs_with_invalid_api_key(self):
        """Test submitting logs with invalid API key."""
        response = self.client.post(
            reverse('submit_logs'),
            data=json.dumps({
    ...
    def test_submit_logs_with_expired_api_key(self):
        """Test submitting logs with expired API key."""
    ...
    def test_report_honeypot_activity_with_valid_api_key(self):
        """Test reporting honeypot activity with valid API key."""
        response = self.client.post(
            reverse('report_honeypot_activity'),
            data=json.dumps({
                'api_key': self.api_key.key,
                'activity': {
                    'type': 'scan',
