...
RUN <<EOF
    set -e
    mkdir -p terracotta/.cargo
    printf '
[target.x86_64-unknown-linux-musl]
linker = "clang"
rustflags = ["-C", "link-arg=-fuse-ld=/usr/local/bin/mold"]' \
        > terracotta/.cargo/config
EOF
...
RUN <<EOF
    ...
    rm src/main.rs
    rmdir src
    target_path=/usr/src/terracotta/target/x86_64-unknown-linux-musl
...
RUN <<EOF
    ...
    if [ "$upx" = "1" ] && [ "$profile" != "dev" ]; then
        upx --best target/x86_64-unknown-linux-musl/$profile/terracotta-simple
    fi
EOF
...
WORKDIR /usr/src
COPY --from=builder /usr/src/terracotta/target/x86_64-unknown-linux-musl/$profile/terracotta-simple ./
COPY Config.toml ./
RUN mkdir content html static

EXPOSE 8000

CMD ["/usr/src/terracotta-simple"]
